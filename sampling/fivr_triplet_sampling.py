...
if __name__ == '__main__':
    ...
    positives = read_text('fivr_positive.txt')
    print(positives)
    save_to='fivr_triplet_0805_margin.csv'
    videos = {v: n for n, v in enumerate(sorted(list(set(list(positives.a) + list(positives.b)))))}
    # bg_videos = {v: n for n, v in enumerate(np.load('/hdd/FIVR_core/fivr_videos_bg.npy')[:1000])}
    bg_videos = {v: n for n, v in enumerate(np.load('/MLVD/FIVR/meta/fivr_videos_bg.npy')[:1000])}
    ...
    df = pd.DataFrame(triplets,
                      columns=['ann_idx', 'scene_start','scene_end', 'anc', 'anc_frame_idx', 'anc_frame', 'pos', 'pos_frame_idx',
