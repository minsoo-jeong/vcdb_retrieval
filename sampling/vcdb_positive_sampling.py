...
def detect_scene(path, fps, min_sec, start=None, end=None):
...
if __name__ == '__main__':
    ...
    for ann_n, ann in enumerate(bar):
        ...
        if a != b:
            ...
            a_scene = detect_scene(a_path, float(a_meta['frame_rate']), 5, start=a_start, end=a_end + 1)
            ...
            b_scene = detect_scene(b_path, float(b_meta['frame_rate']), 5, start=b_start, end=b_end + 1)
    ...
    writer = pd.ExcelWriter('vcdb_positive_candidate.xlsx', engine='xlsxwriter')
